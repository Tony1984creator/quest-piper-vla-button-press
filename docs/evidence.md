...
| CIAT / SmolVLA | Extended a private experiment harness with causal keyframes, exact-prefix replay, paired controls, 15 frozen states, v3/v4 state-held-out absolute-Q versus CRA audits, a 105-pair AFCR axis-direction audit, a fresh BUCED-versus-balanced-axis test, and a fresh multi-scale MACE test. | Offline probe | I turned a Q-gradient idea into auditable offline experiments, recorded that CRA, AFCR, BUCED, and MACE have not established a stable deployable correction signal; the useful result is locating event-sparse keyframes as the bottleneck, while keeping online guidance out of scope. |
