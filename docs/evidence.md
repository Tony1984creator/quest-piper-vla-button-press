...
| CIAT / SmolVLA | Extended a private experiment harness with causal keyframes, exact-prefix replay, paired controls, 15 frozen states, v3/v4 state-held-out absolute-Q versus CRA audits, a 105-pair AFCR axis-direction audit, a fresh BUCED-versus-balanced-axis test, and a fresh multi-scale MACE test, and a matched fresh CST stage-trigger contrast. | Offline probe | I turned a Q-gradient idea into auditable offline experiments, recorded that CRA, AFCR, BUCED, MACE, and CST have not established a stable deployable correction signal. CST matched the legacy trigger at 2/5 direction-changing pairs per arm, so action-history-only stage triggering did not enrich events. The useful result is locating task/visual-stage information as the next boundary, while keeping online guidance out of scope. |
