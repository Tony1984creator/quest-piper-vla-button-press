...
**Status:** v3 and v4 are archived offline baselines. v3 collected 15 states and 129 terminal branches. v4 completed 240/240 primary branches with 15 consistent double-control groups, 134 success / 106 failure outcomes, and 210 candidate/control pairs. In the unified 15-state audit, absolute-Q has terminal MSE 0.3437 and nonzero-event ranking accuracy 0.5133; CRA has advantage MSE 0.0889 and nonzero-event direction accuracy 0.1733. The MSE values use different targets and are not comparable as a winner metric. Only 12 nonzero candidate/control events exist, so CRA v1 has **not** established stable ranking gain. A separately reconstructed AFCR audit then formed 105 complete same-state ±axis pairs: 10 had a direction-changing terminal outcome and 95 tied. It abstained on 49/105 pairs; among its 4 recommended nonzero events it was correct on 0/4. AFCR v1 therefore also provides a negative result, not an axis-direction correction rule. A fresh five-state BUCED test then gave 0/5 direction-changing pairs for both the BUCED-prioritized and balanced axes. A separate fresh five-state MACE test found one direction-changing pair at ±0.05; the remaining four states stayed tied through ±0.25. These small pilots do not show that either acquisition rule improves event discovery. A subsequent fresh five-state CST contrast compared the legacy action-history anchor with a gripper-transition/local-action-peak trigger under matched two-control plus signed ±0.05 budgets. Both arms produced 2/5 direction-changing pairs (40.0%) with all 10 control groups consistent, so CST did not improve event density.

**Next acceptance:** pre-register a task/visual-stage event-enrichment hypothesis before new collection, specify what causal information is permitted and its primary event-density metric, then compare it against the immutable legacy-anchor baseline. The action-history-only trigger branch is closed. Do not add another critic or widen the action magnitude until the trigger produces sufficient non-tied events. Any online Q-gradient action update remains out of scope unless an independent deployment gate is met.
